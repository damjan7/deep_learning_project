...
for epoch in tqdm.tqdm(range(epochs)):
    ...
    for i, (x, y) in enumerate(train_loader):
        ...
        for e in range(num_samples):
            ...
            loss += (F.nll_loss(F.softmax(output, dim=1), y,
                                reduction="sum") + kl_divergence / num_batches) / num_samples
...
def ece(predicted_probabilities: np.ndarray, labels: np.ndarray, n_bins: int = 30) -> float:
    ...
    total_per_bin = np.bincount(probs_as_bin_num, minlength=n_bins) + np.finfo(
        sums_per_bin.dtype).eps  # division by zero
